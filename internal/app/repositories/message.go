...
func (r messageRepository) Create(ctx context.Context, msg models.ChatMessage) (models.ChatMessage, error) {
	...
	if result.Error != nil {
		return models.ChatMessage{}, fmt.Errorf("unable to create message in database: %w", result.Error)
	}
	return msg, nil
}
...
func (r messageRepository) GetMessages(ctx context.Context, roomId, lastSeenMsgId uint64) ([]models.ChatMessage, error) {
	...
	if lastSeenMsgId == 0 {
		...
		if result.Error != nil {
			return nil, fmt.Errorf("unable to retrieve messages for room %d: %w", roomId, result.Error)
		}
		return msgs, nil
	}
